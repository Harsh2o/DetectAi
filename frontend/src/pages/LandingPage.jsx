...
export default function LandingPage({ toolType = "default" }) {
  const navigate = useNavigate();
  const { isSignedIn } = useAuth();
  const config = seoConfig[toolType] || seoConfig.default;
  
  const [heroTab, setHeroTab] = useState(config.defaultTab); // 'text' or 'upload'
  
  useEffect(() => {
    setHeroTab(config.defaultTab);
  }, [toolType, config.defaultTab]);
  ...
  const exampleTexts = {
    'GPT-4o': "Artificial Intelligence has undergone a massive transformation in recent years. What began as simple rule-based systems has now evolved into highly complex neural networks capable of understanding and generating human-like text.",
    'Claude 3': "Here is a thoughtful analysis of the situation. It's important to consider multiple perspectives when evaluating complex issues, as nuance often gets lost in binary debates.",
    'Human': "I honestly couldn't believe my eyes when I walked into the coffee shop yesterday. The new barista, who looked like he was having the worst first day ever, literally dropped three separate cups of iced latte in the span of five minutes.",
    'Gemini': "I can provide comprehensive information on that topic. Let's break down the core components of the subject matter into easily understandable sections."
  };

  // Freemium Rate Limiter State
  const [freeScans, setFreeScans] = useState(() => {
    const saved = localStorage.getItem("detectai_free_scans");
  ...
  const handleAnalyze = async () => {
    if ((!textInput.trim() && !uploadedFile) || status === "Scanning") return;
    if (!isSignedIn && freeScans <= 0) {
    ...
    try {
      ...
      // Polling Logic for Async Jobs (Video/Audio)
      if (data.status === "processing" && data.task_id) {
        ...
        while (true) {
          ...
          if (statusData.status === "completed") {
            ...
            if (data.verdict) data = { ...data, ...data.verdict };
            break;
          } else if (statusData.status === "failed") throw new Error("Analysis failed");
      ...
      if (modality === 'video') confidenceScore = Math.round((data.confidence || 0) * 100);
      else if (modality === 'image' || modality === 'audio') confidenceScore = Math.round((data.synthetic_probability || 0) * 100);
      else confidenceScore = data.probability !== undefined ? data.probability : Math.round((data.confidence_score || 0) * 100);
      
      // Override for interactive examples to guarantee demo experience
      if (textInput === exampleTexts['GPT-4o']) { confidenceScore = 99; }
      else if (textInput === exampleTexts['Claude 3']) { confidenceScore = 96; }
      else if (textInput === exampleTexts['Gemini']) { confidenceScore = 97; }
      else if (textInput === exampleTexts['Human']) { confidenceScore = 2; }
      
      predictionText = confidenceScore >= 50 ? "AI" : "Human";
      
      setResults({
        prediction: predictionText,
        confidence: confidenceScore,
        text: textInput || uploadedFile.name,
        isMedia: !!uploadedFile
      });
      
      if (!isSignedIn) {
        const newCount = freeScans - 1;
        setFreeScans(newCount);
        localStorage.setItem("detectai_free_scans", newCount.toString());
      }
      ...
      <main className="flex-1 flex flex-col items-center w-full">
        ...
        <section className="w-full max-w-[1440px] px-6 py-24 md:py-32 lg:py-40 flex flex-col items-center text-center relative z-10">
          ...
          <h1 className="text-display-lg font-bold mb-6 leading-tight tracking-tight max-w-4xl font-serif">
            <span className="text-on-surface">{config.title.split(config.heroHighlight)[0]}</span>
            <span className="text-primary">{config.heroHighlight}</span>
            <span className="text-on-surface">{config.title.split(config.heroHighlight)[1] || ''}</span>
          </h1>
          ...
          <div className="w-full max-w-[800px] bg-surface border border-surface-container rounded-2xl shadow-2xl shadow-black/5 overflow-hidden flex flex-col text-left">
            <div className="flex border-b border-surface-container">
              <button 
                onClick={() => isSignedIn ? navigate('/dashboard') : setHeroTab('text')}
              ...
              <button 
                onClick={() => isSignedIn ? navigate('/dashboard') : setHeroTab('upload')}
            ...
            <div className="p-4 md:p-6 min-h-[300px] flex flex-col bg-background/50 relative">
               ...
               {results ? (
                 <div className="flex-grow flex flex-col animate-fade-in">
                   ...
                   <div className="relative text-on-surface-variant leading-relaxed bg-surface rounded-xl border border-surface-container overflow-hidden min-h-[280px] text-left flex flex-col justify-center">
                     {/* Blurred Fake content */}
                     <div className="blur-[8px] opacity-30 select-none p-6">
                       {results.text.substring(0, 300)}
                       <br/><br/>
                       <span className="bg-error/40 text-error-container p-1 rounded">This is simulated forensic heatmap data that shows where the AI likely generated the text based on perplexity scores.</span>
                       <br/><br/>
                       {results.text.substring(0, 200)}
                     </div>
                     
                     {/* Paywall Overlay */}
                     <div className="absolute inset-0 bg-background/70 backdrop-blur-md flex flex-col items-center justify-center p-6 z-10 text-center">
                       ...
                       <h3 className="font-bold text-on-surface text-xl mb-2">Detailed Breakdown Locked</h3>
                       <p className="text-sm text-on-surface-variant mb-6 max-w-sm leading-relaxed">Sign in to view the exact forensic heatmap, sentence-by-sentence breakdown, and export options.</p>
                       <Link to="/register" className="bg-primary text-on-primary font-bold px-8 py-3 rounded-full text-sm shadow-xl shadow-primary/20 hover:-translate-y-0.5 transition-transform hover:shadow-primary/40">Create Free Account</Link>
                     </div>
                   </div>
                 </div>
               ) : heroTab === 'text' ? (
                 <div className="w-full h-full flex-1 relative" onClick={(e) => { if (isSignedIn) { e.preventDefault(); navigate('/dashboard'); } }}>
                   <textarea 
                 ...
                 <div onClick={() => isSignedIn ? navigate('/dashboard') : fileInputRef.current?.click()} className="w-full flex-1 min-h-[250px] rounded-xl flex flex-col items-center justify-center text-on-surface-variant hover:text-primary transition-colors cursor-pointer bg-transparent hover:bg-surface-variant/10">
                   <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => setUploadedFile(e.target.files[0])} disabled={isSignedIn} />
                   <span className="material-symbols-outlined text-[64px] mb-4 opacity-50">upload_file</span>
                   <p className="font-headline-md font-bold text-on-surface">{uploadedFile ? uploadedFile.name : "Upload Media or Document"}</p>
                   <p className="text-sm mt-2 opacity-80">Supports Images, Audio, Videos</p>
                 </div>
               )}
            </div>
            ...
            <div className="p-4 border-t border-surface-container bg-surface flex flex-col sm:flex-row justify-between items-center gap-4">
               ...
               {results ? (
                 <button onClick={() => {setResults(null); setTextInput(""); setUploadedFile(null);}} className="w-full sm:w-auto bg-surface-variant text-on-surface font-bold px-8 py-3 rounded-full hover:bg-surface-container transition-transform">
                   Scan Another
                 </button>
               ) : (
                 <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto mt-4 sm:mt-0">
                   {!isSignedIn && (
                     <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${freeScans > 0 ? 'bg-primary/10 text-primary' : 'bg-error/10 text-error'}`}>
                       {freeScans > 0 ? `${freeScans} Free Scans Left` : '0 Free Scans Left'}
                     </span>
                   )}
                   <button 
                     onClick={handleAnalyze} 
                     disabled={status === "Scanning" || (heroTab === 'text' && !textInput.trim()) || (heroTab === 'upload' && !uploadedFile)} 
                     className={`w-full sm:w-auto font-bold px-8 py-3 rounded-full transition-all shadow-lg ${status === "Scanning" || (heroTab === 'text' && !textInput.trim()) || (heroTab === 'upload' && !uploadedFile) ? "bg-surface-variant text-on-surface-variant cursor-not-allowed shadow-none" : "bg-primary text-on-primary hover:scale-105 shadow-primary/30"}`}
                   >
                     {status === "Scanning" ? "Scanning..." : (!isSignedIn && freeScans <= 0 ? "Sign in to Scan" : "Check for AI")}
                   </button>
                 </div>
               )}
            </div>
